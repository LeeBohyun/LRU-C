...
/*======================*/
{
	...
	//fprintf(stderr, "1buf_dblwr_sync_datafiles()\n");
	os_aio_simulated_wake_handler_threads();

	/* Wait that all async writes to tablespaces have been posted to
	the OS */
	//fprintf(stderr, "2buf_dblwr_sync_datafiles()\n");
	os_aio_wait_until_no_pending_writes();

	/* Now we flush the data to disk (for example, with fsync) */
	//fprintf(stderr, "3buf_dblwr_sync_datafiles()\n");
	fil_flush_file_spaces(FIL_TABLESPACE);
	//fprintf(stderr, "4buf_dblwr_sync_datafiles()\n");
}
...
/*=================================*/
{
	...
	if (!srv_use_doublewrite_buf || buf_dblwr == NULL) {
		...
		//fprintf(stderr, "before buf_dblwr_sync_datafiles()\n");
		buf_dblwr_sync_datafiles();
		//fprintf(stderr, "after buf_dblwr_sync_datafiles()\n");
		return;
	}
